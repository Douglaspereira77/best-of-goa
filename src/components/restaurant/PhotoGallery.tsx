      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {photos.slice(0, 6).map((photo, index) => (
          ...
          >
            <Image
              src={photo.url}
              alt={photo.alt || `${restaurantName} photo ${index + 1}`}
              fill
      ...
      <Dialog open={selectedIndex !== null} onOpenChange={closeLightbox}>
        ...
        >
          <DialogTitle className="sr-only">
            {currentPhoto?.alt || `Photo ${(selectedIndex || 0) + 1} of ${photos.length}`}
          </DialogTitle>
            ...
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className="relative w-full h-full p-16">
                <Image
                  ...
                  alt={currentPhoto.alt || `${restaurantName} photo ${selectedIndex + 1}`}
                  fill
              ...
              {currentPhoto.alt && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-50 bg-black/60 text-white px-6 py-3 rounded-full max-w-2xl text-center">
                  <p className="text-sm">{currentPhoto.alt}</p>
                </div>
              )}
            </div>
          )}
