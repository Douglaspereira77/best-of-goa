...
export function SchoolPhotoGallery({ images, schoolName }: SchoolPhotoGalleryProps) {
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {sortedImages.slice(0, 6).map((image, index) => (
          <button
            ...
            className="relative aspect-square rounded-lg overflow-hidden group cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-500"
            aria-label={`View photo ${index + 1} of ${sortedImages.length}`}
          >
            <Image
              src={image.url}
              alt={image.alt_text || `${schoolName} photo ${index + 1}`}
              fill
      ...
      {sortedImages.length > 6 && (
        <button
          ...
          className="mt-4 text-purple-600 font-medium hover:underline focus:outline-none focus:ring-2 focus:ring-purple-500 rounded px-2 py-1"
        >
            ...
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className="relative w-full h-full p-16">
                <Image
                  ...
                  alt={currentImage.alt_text || `${schoolName} photo ${selectedIndex + 1}`}
                  fill
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-50 flex gap-2">
                {currentImage.is_hero && (
                  <div className="bg-purple-600 text-white text-xs px-3 py-1.5 rounded-full font-medium">
                    Hero Image
                  </div>
                )}
                {currentImage.type && (
