...
export async function GET(request: NextRequest) {
  ...
  try {
    const snapshot = await adminDb.collection('malls').where('active', '==', true).limit(100).get();
    ...
    console.error('Malls search error:', error);
    return NextResponse.json({ error: 'Failed to search malls' }, { status: 500 });
  }
}
