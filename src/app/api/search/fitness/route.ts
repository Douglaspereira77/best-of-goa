...
export async function GET(request: NextRequest) {
  ...
  try {
    const snapshot = await adminDb.collection('fitness_places').where('active', '==', true).limit(100).get();
    ...
    console.error('Fitness search error:', error);
    return NextResponse.json({ error: 'Failed to search fitness places' }, { status: 500 });
  }
}
