 ...
 *
 * Searches Google Places API for hotels based on query string
 * Used in admin panel to find hotels to add to the directory
 *
 * Enhanced: Now checks each result against existing hotels in database
...
export async function POST(request: NextRequest) {
  try {
    ...
    searchUrl.searchParams.append('type', 'lodging'); // Filter to hotels/lodging only
    ...
    // Check for existing hotels in database (proactive duplicate detection)
    ...
    const existingMap = new Map(
      (existingHotels || []).map(hotel => [hotel.google_place_id, hotel])
    );
