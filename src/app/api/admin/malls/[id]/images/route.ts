...
// GET - Fetch all images for a mall
  ...
  try {
    ...
    if (shouldAnalyze) {
      try {
        ...
        imageAnalysis = await openai.analyzeImage(buffer, mall.name, 'mall')
      } catch (analysisError) {
    ...
    const filename = `${mall.slug}-${filenameBase}.${ext}`
    const path = `malls/${mall.slug}/images/${filename}`
    ...
    const finalAltText = altText || imageAnalysis?.alt_text || `${mall.name} image`
...
export async function DELETE(
  ...
  try {
    ...
    if (image.metadata?.path) {
      await supabase.storage.from('malls').remove([image.metadata.path])
    }
