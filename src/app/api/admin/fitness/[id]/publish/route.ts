...
export async function POST(
  ...
  try {
    ...
    if (fetchError || !fitness) {
      return NextResponse.json(
        { success: false, error: 'Fitness place not found' },
        { status: 404 }
      )
    }
    ...
    if (updateError) {
      console.error('Error updating fitness place published status:', updateError)
      return NextResponse.json(
        { success: false, error: 'Failed to update fitness place status' },
        { status: 500 }
      )
    }

    const message = newPublishedStatus
      ? `Fitness place "${fitness.name}" published successfully`
      : `Fitness place "${fitness.name}" unpublished successfully`
