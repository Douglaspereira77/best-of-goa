...
// GET - Fetch all images for a fitness place
  ...
  try {
    ...
    if (shouldAnalyze) {
      try {
        ...
        imageAnalysis = await openai.analyzeImage(buffer, fitness.name, 'fitness')
      } catch (analysisError) {
    ...
    const filename = `${fitness.slug}-${filenameBase}.${ext}`
    const path = `fitness/${fitness.slug}/images/${filename}`
    ...
    const finalAltText = altText || imageAnalysis?.alt_text || `${fitness.name} image`
...
export async function DELETE(
  ...
  try {
    ...
    if (image.metadata?.path) {
      await supabase.storage.from('fitness').remove([image.metadata.path])
    }
