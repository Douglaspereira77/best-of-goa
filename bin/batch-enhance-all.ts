 ...
 *
 * Usage:
 *   npx ts-node bin/batch-enhance-all.ts [--dry-run] [--limit=10] [--restaurant-id=xxx]
 *
 * Options:
...
import { ImageExtractor } from '../src/lib/services/image-extractor';
  ...
  for (let i = 0; i < restaurants.length; i++) {
    ...
    try {
      ...
      if (!skipImages && restaurant.photos && restaurant.photos.length > 0) {
        ...
        const needsAnalysis = restaurant.photos.some((p: any) => !p.alt || !p.title);
